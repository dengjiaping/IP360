...
<<<<<<< HEAD
	...
	public ResuambleUpload(OSS client, String testBucket, String testObject,
			String uploadFilePath, int resourceid) {
	...
	public void resumableUploadWithRecordPathSetting() {
		...
		final String token = (String) SharePreferenceUtil.getAttributeByKey(
				MyApplication.getApplication(), MyConstants.SP_USER_KEY,
				"token", SharePreferenceUtil.VALUE_IS_STRING);
		// 创建断点上传请求，参数中给出断点记录文件的保存位置，需是一个文件夹的绝对路径
		ResumableUploadRequest request = new ResumableUploadRequest(testBucket,
				testObject, uploadFilePath, recordDirectory);
...
//				put("callbackUrl","http://101.201.74.230:9091/api/v1/file/uploadFileOssStatus");
				put("callbackUrl","101.201.74.230:9091/api/v1/file/uploadFileOssStatus");
				put("callbackBody","resourceid=${x:resourceid}&token = ${x:token}");

			}

		});
		LogUtils.e("token" + token + "resourceid" + resourceid);
						...
						new OSSCompletedCallback<ResumableUploadRequest, ResumableUploadResult>() {
							...
							public void onSuccess(
								...
								if (progressListener != null) {
									...
									UpDownLoadDao.getDao()
											.deleteUploadInfoByUrl(
													uploadFilePath);
								}
							}
		 ...
		 * 
		 * @Override public void run() { // TODO Auto-generated method stub File
		 * file=new File(uploadFilePath); File file1=new
		 * File(Environment.getExternalStorageDirectory
		 * ()+"/ip360/test/test1.jpg"); try { FileOutputStream fos=new
		 * FileOutputStream(file1); FileInputStream fis=new
		 * FileInputStream(file); int len; int total=0; byte[] buffer=new
		 * byte[1024]; try { while (iscancel&&(len=fis.read(buffer))!=-1) {
		 * fos.write(buffer, 0, len); progress+=len; Log.i("djj", progress+"");
